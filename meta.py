...
class Meta(nn.Module):
    ...
    def predict(self, image):
        ...
        fw = torch.load("fastweight.pth", weights_only=False, map_location=device)
        ...
        return F.softmax(self.net(image, self.fast_weights, bn_training=True), dim=1)
    ...
    def finetunning(self, x_spt, y_spt, x_qry, y_qry, TorF, denorm_func, num):
        ...
        with torch.no_grad():
            ...
            logits_q = net(x_qry, net.parameters(), bn_training=True, region_weights=self.values)
            # [setsz]
            pred_q = F.softmax(logits_q, dim=1).argmax(dim=1)
        ...
        with torch.no_grad():
            ...
            logits_q = net(x_qry, fast_weights, bn_training=True, region_weights=self.values)
            # [setsz]
            pred_q = F.softmax(logits_q, dim=1).argmax(dim=1)
        ...
        for k in range(1, self.update_step_test):
            ...
            logits = net(x_spt, fast_weights, bn_training=True, region_weights=self.values)
            loss = F.cross_entropy(logits, y_spt)
            # 2. compute grad on theta_pi
            grad = torch.autograd.grad(loss, fast_weights)
            # 3. theta_pi = theta_pi - train_lr * grad
