...
class Meta(nn.Module):
    ...
    def finetunning(self, x_spt, y_spt, x_qry, y_qry, TorF):
        ...
        torch.save(fast_weights, net_save_path) # save the trained model
        fw = torch.load("fastweight.pth", weights_only=True)
        
        torch.save(net.state_dict(), f"{current_file_path}/net.pth")    # save the trained model

        net_test = deepcopy(self.net)
        pred = net_test(x_qry, fw, bn_training=True)
        with torch.no_grad():
