...
class Meta(nn.Module):
    ...
    def finetunning(self, x_spt, y_spt, x_qry, y_qry, TorF):
        ...
        self.fast_weights = list(map(lambda p: p[1] - self.update_lr * p[0], zip(grad, net.parameters())))
        accc=[]
        precision=[]
        F1=[]
        Recall=[]
        ...
        with torch.no_grad():
            # [setsz, nway]
            logits_q = net(x_qry, self.fast_weights, bn_training=True)
            # [setsz]
            pred_q = F.softmax(logits_q, dim=1).argmax(dim=1)
            # scalar
            correct = torch.eq(pred_q, y_qry).sum().item()
        ...
        for k in range(1, self.update_step_test):
            # 1. run the i-th task and compute loss for k=1~K-1
            logits = net(x_spt, self.fast_weights, bn_training=True)
            loss = F.cross_entropy(logits, y_spt)
            # 2. compute grad on theta_pi
            grad = torch.autograd.grad(loss, self.fast_weights)
            # 3. theta_pi = theta_pi - train_lr * grad
            self.fast_weights = list(map(lambda p: p[1] - self.update_lr * p[0], zip(grad, self.fast_weights)))
        ...
        if TorF:
            ...
            net_save_path = f"{current_file_path}/fast_weights.pth"
            torch.save(self.fast_weights, net_save_path)
